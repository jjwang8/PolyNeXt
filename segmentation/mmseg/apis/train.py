...
class BackboneUnfreezeHook(Hook):
    ...
    def before_run(self, runner):
        ...
        runner.logger.info(f'[BackboneUnfreezeHook] Backbone frozen for first {self.unfreeze_iter} iters')
    ...
    def before_train_iter(self, runner):
        if runner.iter >= self.unfreeze_iter and not self.unfrozen:
            ...
            self.unfrozen = True
            runner.logger.info(f'[BackboneUnfreezeHook] Backbone unfrozen at iter {runner.iter}')

@HOOKS.register_module()
class SafeOptimizerHook(OptimizerHook):
    """OptimizerHook that zeros NaN/Inf gradients before stepping."""
    ...
    def after_train_iter(self, runner):
        ...
        if nan_count > 0 and runner.iter % 50 == 0:
            runner.logger.warning(f'Iter {runner.iter}: zeroed NaN/Inf grads in {nan_count} params')

        # Grad clipping if configured
...
def train_segmentor(model,
    ...
    # runner.register_hook(BackboneUnfreezeHook(unfreeze_iter=1000))
    # runner.register_hook(runner.register_hook(SafeOptimizerHook(grad_clip=dict(max_norm=1.0)), priority='ABOVE_NORMAL'))
